 ...
 *
 * Scroll-stacked deck: the first monastery fills the view on arrival, and
 * each subsequent one rides up over the last as you scroll. Restored here
 * after a merge favoured an older two-column layout that had neither the
 * stacking nor the legibility-fixed image scrim — see StackedCards.tsx and
 * the explicit-stop gradient below for why both matter.
 *
 * Wired to the existing `explore.*` / `monasteries.*` translation keys,
 * already fully translated in hi/ne, rather than new ones — the headline
 * uses the existing explore.title/subtitle copy so the redesign gets
 * working Hindi and Nepali immediately instead of needing fresh strings.
 */
const Explore = () => {
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] font-medium uppercase tracking-[0.28em] text-heritage">
                {t("explore.label", "The Buddhist Circuit")}
              </span>
            </div>
            ...
            <div className="mt-8 grid gap-8 border-t border-foreground/10 pt-8 md:grid-cols-12">
              <h1 className="font-display text-5xl leading-[1.05] tracking-tight text-foreground md:col-span-7 md:text-7xl">
                {t("explore.title", "Beyond Temples, Beyond Time")}
              </h1>
              <p className="text-base leading-relaxed text-muted-foreground md:col-span-5 md:pt-3">
                {t(
                  "explore.subtitle",
                  "The history, culture and geography behind each site — and a narrated tour through all of them."
                )}
              </p>
            </div>
          </motion.div>
        ...
        <div className="container mx-auto mt-16 px-6 lg:px-8">
          <StackedCards>
            {locations.map((site, index) => {
              ...
              const founded = timelineEntries.find((entry) => entry.id === site.id);
                    ...
                    <div className="relative flex h-full flex-col justify-between p-7 md:p-12">
                      ...
                      <div className="max-w-2xl">
                        ...
                        <h2 className="mt-3 font-display text-4xl leading-[1.02] tracking-tight text-white md:text-6xl">
                          {t("monasteries." + site.id + ".name", site.name).replace(" Monastery", "")}
                        </h2>
                        ...
                        <dl className="mt-7 flex flex-wrap gap-x-10 gap-y-4 border-t border-white/15 pt-5">
                          ...
                          {story && (
                            <div>
                              <dt className="text-[10px] uppercase tracking-[0.2em] text-white/45">
                                {t("explore.storyTour", "Story tour")}
                              </dt>
                              <dd className="mt-1 font-mono text-lg tabular-nums text-white">
                                {t("explore.chapters", "{{count}} chapters", { count: story.chapters.length })}
                              </dd>
                            </div>
                          )}
                        </dl>
                        ...
                        <div className="mt-7 flex flex-wrap gap-3">
                          ...
                          >
                            ...
                            {t("heritage.label", "Heritage Trails")}
                            <ArrowUpRight
        ...
        <div className="container mx-auto px-6 pb-24 lg:px-8">
          <div className="flex flex-col items-center gap-5 border-t border-foreground/10 pt-14 text-center">
            ...
            >
              ...
              {t("explore.virtualExperience", "Virtual experience")}
            </Link>
          </div>
        </div>
      </div>
